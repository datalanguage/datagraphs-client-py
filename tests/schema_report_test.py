"""Tests for the net-effect change reporting pipeline (datagraphs.schema_report).

Covers the pure diff/annotate/render machinery — ``_diff``, ``_replay_identities``,
``_annotate``, and the ``change_report()`` / ``build_change_report()`` output
contract — plus the adversarial-review regression suites. Schema mutation, op-log
recording, baseline capture, and atomic rollback are exercised in
...
class TestChangeDiff:
    ...
    def test_date_fields_never_appear_in_any_change_via_schema_mutations(self):
        ...
        changes = _diff(s._tracker.baseline, s._schema)
        self._assert_no_date_fields(changes)
...
class TestChangeRenames:
    ...
    def _reconcile(self, s: DatagraphsSchema) -> list[Change]:
        ...
        rename_map = _replay_identities(s._tracker.baseline, s._tracker.change_log)
        return _diff(s._tracker.baseline, s._schema, rename_map=rename_map)
    ...
    def test_fold_property_rename_yields_baseline_keyed_entry(self):
        ...
        rm = _replay_identities(s._tracker.baseline, s._tracker.change_log)
        assert rm.classes == {}
    ...
    def test_diff_without_rename_map_treats_rename_as_remove_add(self):
        s = self._schema()
        s.rename_property("Drug", "dosage", "dose")
        changes = _diff(s._tracker.baseline, s._schema, rename_map=None)
        prop_ops = {(ch.target, ch.op) for ch in changes if ch.kind == "property"}
        assert ("Drug.dosage", "removed") in prop_ops
        assert ("Drug.dose", "added") in prop_ops
        assert not [ch for ch in changes if ch.op == "renamed"]
...
class TestChangeSemanticAnnotation:
    ...
    def _annotated(self, s: DatagraphsSchema) -> list[Change]:
        ...
        rename_map = _replay_identities(s._tracker.baseline, s._tracker.change_log)
        changes = _diff(s._tracker.baseline, s._schema, rename_map=rename_map)
        return _annotate(changes, s._tracker.change_log, s._tracker.baseline, s._schema, rename_map=rename_map)
    ...
    def test_empty_change_log_leaves_changes_unchanged(self):
        ...
        rename_map = _replay_identities(s._tracker.baseline, s._tracker.change_log)
        diffed = _diff(s._tracker.baseline, s._schema, rename_map=rename_map)
        annotated = _annotate(diffed, s._tracker.change_log, s._tracker.baseline, s._schema, rename_map=rename_map)
...
class TestChangeReportRendering:
    ...
    def test_change_report_does_not_mutate_change_log(self):
        ...
        log_snapshot = copy.deepcopy(s._tracker.change_log)
        s.change_report()
        s.change_report()
        assert s._tracker.change_log == log_snapshot
    ...
    def test_change_report_does_not_mutate_baseline(self):
        ...
        s = self._happy_path_schema()
        baseline_snapshot = copy.deepcopy(s._tracker.baseline)
        s.change_report()
        s.change_report()
        assert s._tracker.baseline == baseline_snapshot
...
class TestReviewFindings3:
    ...
    @pytest.mark.skip(reason="This is a known limitation of the text format, but right now we do now use property descriptions which contain newlines. We can unskip and xfail this test if we add a property description with a newline in the future.")
